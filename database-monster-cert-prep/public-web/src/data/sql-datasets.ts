// Canonical in-memory dataset shared by the SQL labs and `sql-write` exam
// questions. A fresh SQLite database is seeded from SEED_SQL on every run, so
// DDL/DML/trigger exercises stay isolated and never pollute later runs.
//
// Notes baked into the data on purpose (so the labs have something to find):
//   - some students have a NULL email           -> filtering / IS NULL
//   - some students have no enrollments          -> LEFT JOIN / NOT EXISTS
//   - one course has zero enrollments            -> anti-join exercises
//   - grades include a NULL                      -> AVG ignores NULL
// The `mentors` table is intentionally NOT seeded — lab 06 creates it.
...
INSERT INTO enrollments (enrollment_id, student_id, course_id, grade) VALUES
  ...
  (10, 10, 4, 67),
  (11, 1, 5, 91);
-- students 4, 6, 8 have no enrollments (LEFT JOIN / NOT EXISTS exercises).
...
// Add a course with zero enrollments for anti-join exercises.
export const SEED_SQL_FULL =
  SEED_SQL +
  `INSERT INTO courses (course_id, course_code, title, department_id) VALUES (6, 'CS499', 'Capstone', 1);\n`;

// Human-readable schema summary, reused in lab/question UI.
